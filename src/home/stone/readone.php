<?php
 ...
 $conStone=mysql_connect("127.0.0.1:xxxx", "<db-username>", "<db-password>");
      ...
      $username = '';
	  $_SESSION['stop']=0;
	  $_SESSION['stoneid'] =$_GET['showstory'];
	  $indtime = time() + 45000;
     ...
     $pro2 = mysql_fetch_row(mysql_query("SELECT projected FROM profiles WHERE userID = '$uid' AND projected =1",$con));
	...
	$_SESSION['url_temp']='';	
      ...
      $uidT = mysql_fetch_row(mysql_query("SELECT userID from stone WHERE stoneID = '$_GET[showstory]'",$conStone));	  
	  $read = mysql_fetch_row(mysql_query("SELECT * FROM readerdata WHERE (stoneID = '$_GET[showstory]') AND (readerID='$uid') AND (action='r')",$conStone));
	   $inf =mysql_fetch_row(mysql_query("SELECT * FROM readerdata WHERE (stoneID = '$_GET[showstory]') AND (readerID='$uid') AND (action='i')",$conStone));
	if($_GET['read']==1)
   {
		if($read==null)
	{
		mysql_query("UPDATE stone SET readCount = readCount+1 WHERE stoneID = '$_GET[showstory]'",$conStone);
		mysql_query("INSERT INTO readerdata (stoneID, authorID, readerID, action) VALUES ('$_GET[showstory]', '$uidT[0]', '$uid', 'r')",$conStone);
		mysql_query("INSERT INTO notification (writer, reader, action,time) VALUES ('$uidT[0]', '$uid', 'r','$time')",$con);
		$readit =1;
   ...
   if($_GET['inf']==1)
   {
		if($inf==null)
	{
		mysql_query("UPDATE stone SET benefitCount = benefitCount+1 WHERE stoneID = '$_GET[showstory]'",$conStone);
		mysql_query("INSERT INTO readerdata (stoneID, authorID, readerID, action) VALUES ('$_GET[showstory]', '$uidT[0]', '$uid', 'i')",$conStone);
		mysql_query("INSERT INTO notification (writer, reader, action,time) VALUES ('$uidT[0]', '$uid', 'i','$time')",$con);
		$infit =1;
   ...
   function linker($in)
	{
	 $linkpatt='/\b(https*:\/\/)+[a-zA-Z0-9\-]+\.[a-zA-Z0-9\-]+\.*[^\s]+/';
					  preg_match($linkpatt, $in, $matches);
					  $replace ="<a href='$matches[0]'>$matches[0]</a>";
					  $stowithlink = str_ireplace($matches[0], $replace, $in);
					  return $stowithlink;
	}
	
	 $gets = mysql_fetch_array(mysql_query("SELECT * FROM stone WHERE stoneID = '$_GET[showstory]'",$conStone));
?>
<!DOCTYPE html PUBLIC "-//W3C//DTD html 4.01 transitional //EN" "http://www.w3.org/TR/html4/loose.dtd">
<HTML class="kornflakesSearch" id="profileIndex" lang="en">
  <HEAD>
      <TITLE>  <?php echo $gets['title']; ?> : Stone</TITLE>
  ...
  <BODY class="kornContent" onload="startNotification(), updatenotif(), loadcomment(''), loginstatus()">
  
    <DIV class="mainContainer">
	    
	    <DIV class="stonetitle">STONE<span style="font-weight:normal;">&trade;</span> </DIV>
	    <DIV class="stonecontent">
		<?php $getnum = mysql_num_rows(mysql_query("SELECT * FROM stone",$conStone)); ?>
		<DIV class="readhead"><br /><span onclick="location.href='read.php';" style="cursor:pointer;color:blue;"> All <?php echo $getnum; ?> stories </span>
		 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		 Search stories by title, campus or author : 
		 <input type="text" id="stonesearch" onkeyup="stonesearch(0,10)" />
		  ...
		  <DIV id="getstone"></div>
		  <DIV id="extra">
              <?php if(isset($_GET['showstory']))
					  {
					  
					 if($_GET['showstory']!=2 || ($uid==56 || $uid==220))
					{ $pro = mysql_fetch_row(mysql_query("SELECT projected FROM profiles WHERE userID = '$gets[userID]' AND projected =1",$con));
					  $stowithlink = linker($gets['story']);
					  ?>
					  <DIV style="padding:7px; border-bottom: 1px solid #aaaaaa;">
					   <DIV style="font-weight:bold;font-size:22px;"> <?php echo $gets['title']; ?></DIV> 
						 <DIV style="color:#909090; font-size:13px;font-family:verdana;"><?php echo $gets['brief']; ?></DIV><br />
					   <DIV style="font-color:#606060;font-size:15px;"> - by 
					   <?php if($pro != null)
					    { ?>
					   <span id="prof" onclick="location.href='http://kf.ahens.com/home/users/showProfile.php?userID=<?php echo $gets['userID']; ?>';"><?php echo $gets['name']; ?></span> 
					   <?php }
					   else echo $gets['name'];
						if($gets['userID']==$uid) 
							{ ?> | <span id='prof' onclick="location.href='share.php?edit&id=<?php echo $gets['stoneID']; ?>';"> Edit </span> <?php }
					   ?>
					   </DIV>
					  
					   <DIV style="font-family:verdana;font-size:13px;color:#909090; font-style:italic;"> <?php echo $gets['campus']; ?> </DIV>
					   <DIV style="font-size:12px;color:#bbbbbb;"> <?php echo $gets['time']; ?> </DIV>
					   <br /></DIV>
					   <DIV style="padding:7px; line-height:180%; font-size:13px; color:#505050;border-bottom: 1px solid #dddddd;font-family:'trebuchet ms',tahoma;">
						<?php echo nl2br($stowithlink); ?>
						</DIV><br /><DIV style="font-size:13px;color:#808080;border-bottom:solid 1px #aaaaff;">
						<?php if($gets['place']=='' && $gets['sources']=='')
								 { ?> Original work of <?php
								       if($pro !=null) {
									    ?>
										<span style="font-weight:bold;"id="prof" onclick="location.href='http://kf.ahens.com/home/users/showProfile.php?userID=<?php echo $gets['userID']; ?>';"><?php echo $gets['name']; ?></span>
										  <?php }
										  else
										     { ?> <span style="font-weight:bold;"> <?php echo $gets['name']; ?> </span> <?php }
									 }
								else
								  {
								  $source = linker($gets['sources']);
                                  echo "Place : <span style='font-weight:bold;'>".$gets['place']."<br /></span>";
								  echo "Source(s) : <span style='font-weight:bold;'>".$source."</span>";
									} ?>
						<br /><br />		
						</DIV><br /><br />
						<DIV style="text-align:center; font-family;verdana; font-size:12px;color:#1C85D4;border-bottom:solid 1px #aaaaff;">
						<?php if($read==null && $readit!=1)
						  { ?>
						 Read by <span style='font-size:15px;'><?php echo $gets['readCount']; ?></span> <span id="prof" onmouseover="loadinf(0)" style="color:blue;">people</span>. <span id="prof" onclick="location.href='readone.php?showstory=<?php echo $_GET['showstory']; ?>&read=1';">Mark as read.</span>
						  <?php } 
						  else
						     echo "Read by you and <span style='font-size:15px;'>".($gets['readCount']-1)."</span> other <span id='prof' style='color:blue;' onmouseover='loadinf(0)'>people</span>.";
							?> 
							<br /><br /> <?php
						   if($inf==null && $infit!=1)
							{ ?>
							 <span style="font-size:15px;"><?php echo $gets['benefitCount']; ?></span> <span id="prof" style="color:blue;" onmouseover="loadinf(1)">people</span> were influenced by this story. <span id="prof" onclick="location.href='readone.php?showstory=<?php echo $_GET['showstory']; ?>&inf=1';"> Were you?</span>
						   <?php }
						   else
							 echo "You and <span style='font-size:15px;'>".($gets['benefitCount']-1)."</span> other <span id='prof' onmouseover='loadinf(1)' style='color:blue;'>people</span> were influenced by this story.";
						   ?>
						   <br /><br /><br />
						 </DIV>
						<DIV class="comment">Comments & Discussions<hr />
						 <DIV id="comment"></DIV><br /><DIV class="current"> <?php
						 if($pro2 != null)
							{ ?>
							<span style="font-weight:bold;" id="prof" onclick="location.href='http://kf.ahens.com/home/users/showProfile.php?userID=<?php echo $uid; ?>';"><?php echo $username; ?>
							</span>
							<?php }
							else { ?>
							  ...
							  <?php	} ?> 
							<br />
								<input type="text" name="comm" id="cmnt" style="width:530px; height:25px;font-family:tahoma;" onkeyup="enter(this.value,event)" />
						 </DIV>
						</DIV>  
					<?php }	
					}	?>
		  <br /></DIV>
		  <DIV id="loadspecial"></DIV>
		  <DIV id="getmore"></DIV>
		...
		<br /><br />
		<div class="active">Read
		<div id="arrow"></div><div id="arrowin"></div></DIV>
		<br /><br /><br />
...
<!--	<DIV class="menu" id="menu" onmouseover="menuOver()" onmouseout="menuOut()">
	...
	<DIV style="font-weight:bold;" class="sideLink2" id="sideLnk6"  onmouseover="sidelinkover('sideLnk6')" onmouseout="sidelinkout('sideLnk6','The New Home')" onclick="location.href='http://kf.ahens.com/home/home.php';">
	 ...
	 <span style="font-size:15px;"> Where would we have been, if there were no softwares?</span>
